...
bool BlipKitInstrument::_set(const StringName &p_name, const Variant &p_value) {
	if (p_name.begins_with("sequence/")) {
		...
		if (name == "sequence/pitch") {
			sequence = SEQUENCE_PITCH;
		} else if (name == "sequence/volume") {
			sequence = SEQUENCE_VOLUME;
		} else if (name == "sequence/panning") {
			sequence = SEQUENCE_PANNING;
		} else if (name == "sequence/duty_cycle") {
			sequence = SEQUENCE_DUTY_CYCLE;
		} else {
			return false;
		}
