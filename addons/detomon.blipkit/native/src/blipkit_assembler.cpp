...
bool BlipKitAssembler::put_cmd(Instruction p_instr, const Command &p_cmd) {
	...
	switch (p_instr) {
		...
		default: {
			int byte_offset = bytes->get_position();
			ERR_FAIL_V_MSG(false, vformat("Invalid instruction %s at byte offset %d.", p_instr, byte_offset));
		} break;
	}
...
bool BlipKitAssembler::expect_args(const Command &p_cmd, Variant::Type p_type_1, Variant::Type p_type_2, Variant::Type p_type_3) {
	...
	for (int i = 0; i < 3; i++) {
		if (p_cmd.args[i].get_type() != types[i]) {
			...
			int byte_offset = bytes->get_position();
			ERR_FAIL_V_MSG(false, vformat("Expected argument %d to be type %s at byte offset %d.", i + 1, type_name, byte_offset));
		}
	}
...
void BlipKitAssembler::put(Instruction p_instr, const Variant &p_arg_1, const Variant &p_arg_2, const Variant &p_arg_3) {
	put_cmd(p_instr, { .args = { p_arg_1, p_arg_2, p_arg_3 } });
}
...
bool BlipKitAssembler::put_label(String p_label) {
	...
	if (label.byte_offset >= 0) {
		int byte_offset = bytes->get_position();
		ERR_FAIL_V_MSG(false, vformat("Label '%s' is already defined at byte offset %d.", p_label, byte_offset));
	}
...
PackedByteArray BlipKitAssembler::compile() {
	...
	for (Address &address : addresses) {
		...
		if (label.byte_offset < 0) {
			...
			compile_error = vformat("Label '%s' not defined at byte offset %d.", label.name, address_offset);
			ERR_FAIL_V_MSG(PackedByteArray(), compile_error);
		}
