...
**Generates bytecode from instructions.**

## Description

Generates bytecode from instructions which can be executed with [`BlipKitInterpreter`](BlipKitInterpreter.md).
...
# Compile bytecode and check for errors.
...
Resolves label addresses and generates the bytecode.

Call [`get_byte_code()`](#blipkitbytecode-get_byte_code) to get the bytecode. Call [`clear()`](#void-clear) to generate new bytecode.

Returns [`ERR_INVALID_STATE`](#err_invalid_state) if the bytecode is already compiled.
...
Adds compiled [`BlipKitBytecode`](BlipKitBytecode.md) and returns [`OK`](#ok) on success. Public labels from `bytecode` are added to the existing labels.

If `public` is `true`, public labels from `bytecode` are also made public.

Returns [`ERR_DUPLICATE_LABEL`](#err_duplicate_label) if a label in `bytecode` already exists.

Returns [`ERR_INVALID_ARGUMENT`](#err_invalid_argument) if `bytecode` is not valid.
...
If `public` is `true`, the label can be used as entry point when loading bytecode with `BlipKitInterpreter.load_byte_code()`.
